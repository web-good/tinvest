...
// 34.5 месяца, канон 36/12/6 не помещается. Схема адаптирована до 34/9/6 — четыре фолда встык с
// запасом в месяц, жертвуется длина обучающего окна, а не число фолдов, потому что правило сборки
// точки «≥3 фолда из 4» при трёх фолдах вырождается. Прецеденты: IVAT 25/9/4, DIAS 30/10/5, YDEX
// 25/9/4. ПОПРАВКА 2026-08-31: изначальная схема 34/10/6 (встык без запаса) на живом прогоне
// 31-го числа дала три фолда вместо четырёх — AddDate от time.Now() переполняет конец месяца и
// роняет правый край четвёртого фолда за today (cmd/backtest/main.go, walkforward.go). Обучающее
// окно укорочено на месяц, чтобы фолды не зависели от даты запуска.
