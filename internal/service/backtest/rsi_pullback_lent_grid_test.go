...
func TestLENTSignalGridsPinTheirMeasuredAxes(t *testing.T) {
	...
	// Потолок 25, а НЕ 50 как в wush/. Ось WUSH подняли, проверяя догадку об обрезании, и догадку
	// опровергли замером: pooled OOS PF упал с 2.000 до 1.674, а три фолда из четырёх остались в
	// прежней глубокой зоне. Копировать сюда широкую ось значит копировать проваленный
	// эксперимент. На LENT граница подтверждена своим замером: RSI(4) уходит под 30 уже 1690 раз
	// по будням, под 40 — 2671, то есть за 25 порог описывает не откат, а обычное дыхание цены.
	for _, v := range entry["RSILower"] {
		if v > 25 {
			t.Errorf("cal_entry.json свипует RSILower=%v: глубже 25 это уже не откат (RSI(4) уходит под 30 1690 раз по будням); расширение той же оси на WUSH результат УХУДШИЛО", v)
		}
	}
	...
	// RSIUpper здесь не свипуется: полоса выхода меряется отдельно, файлом cal_exit.json. На
	// WUSH владелец свёл обе оси в одну тему и получил 54 комбинации на четырёх фолдах; сюда это
	// решение не переносится — 4x4x6 = 96 комбинаций на одной теме переобучаются по построению.
