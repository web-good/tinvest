...
func TestWUSHSignalGridsPinTheirMeasuredAxes(t *testing.T) {
	...
	// Потолок 50, а не 25, как было в первой редакции. Ось расширили, проверяя догадку об
	// обрезании (калибратор выбирал 20/20/20/25, упираясь в верхний край), и догадку ОПРОВЕРГЛИ:
	// на девяти уровнях выбор стал 40/20/20/25, а pooled OOS PF упал с 2.000 до 1.674. Широкой
	// ось оставлена намеренно — чтобы не предполагать ответ, — но сужать её обратно к 25 значит
	// вернуть ту же слепую зону, поэтому тест держит именно широкий потолок. Выше 50 двигать
	// нечего: между 45 и 50 у RSI(4) прибавляется 7 кроссов из 2936 (0.2%), ось насыщена, а 50 —
	// средняя линия осциллятора, выше которой отката нет по определению.
	for _, v := range entry["RSILower"] {
		if v > 50 {
			t.Errorf("cal_entry.json свипует RSILower=%v: выше 50 отката нет по определению (50 — средняя линия RSI), и ось насыщена уже на 45", v)
		}
	}
	// Ниже 10 выборка истончается быстрее, чем растёт качество сигнала: у RSI(7) на уровне 10
	// остаётся 64 будних кросса за всю историю, и более глубокий порог режет и их.
	for _, v := range entry["RSILower"] {
		if v < 10 {
	...
	// 116 будних кроссов RSI(6)@10 эту точку выдерживают. На DOMRF таких кроссов было 18, и там
