...
// Жёсткие основания отсева по данным: нет дивиденда (yield <= 0), низкая
// ликвидность (MarketCapitalization < MinMarketCap, в т.ч. 0 из-за proto3
// omitempty) и тонкий free-float (FreeFloat < MinFreeFloat, в т.ч. 0).
// Отсутствие EBITDA/payout (0) НЕ исключает компанию — оно нейтрально
// учитывается в пиллярах.
// gate — воротный фильтр вселенной. Внимание: gate работает ДО секторной ветки
// в Rank и намеренно секторно-слеп. Долговые рычаги ниже (yield-trap по
// NetDebtToEbitda > 3 и high-leverage по MaxNetDebtToEbitda) применяются и к
// SectorFinancial: на живых данных API отдаёт NetDebt/EBITDA = 0 для всех
// финансовых имён, поэтому рычаги для них не срабатывают и до Safety=0.5 доходит
// каждое. Если API начнёт возвращать реальный долг для финансовых холдингов
// (напр. АФК «Система», код "financial"), такое имя будет отсеяно по долгу
// раньше, чем сработает секторная ветка «долг неприменим» — это осознанный
// компромисс v1 (различия «банк vs холдинг» классификатор пока не делает).
