...
тике. Пакет: `internal/service/trading_strategy/reversion/live/stoporders`.
...
### Бэкстоп через проверку ядра

Помимо резидентной биржевой заявки, каждый manage-тик заново вызывает `core.Decide` на
последней закрытой часовой свече — и он **тоже** проверяет условие STOP (`low` бара ≤
уровню). Если по какой-то причине биржевая заявка не сработала вовремя, пропущена или её
вообще нет (dry-run, временный провал `Place`), но цена всё равно ушла ниже уровня, ядро
на следующем часовом тике само вернёт `SignalSell` с причиной `SL`/`ATRSL`/`TRAIL`, и runner
продаст рыночным ордером по общему пути «cancel перед любой рыночной продажей» (см. выше).
Это резервный контур: он реагирует не быстрее часового расписания, но гарантирует, что
...
- **Остальные (`OB`/`RSI50`/`BE`/`RSIOS`/`EMAX`):**
...
- **Выход:** `🔴 Выход UGLD [OB]\n  Цена: 2857.8000 | Штук: 35` (в скобках — код причины:
  `SL`/`ATRSL`/`TRAIL` — компоненты STOP, интрабарные — плюс `OB`/`RSI50`/`BE`/`RSIOS`/`EMAX`
  — close-based; полный порядок приоритета и семантика каждого — в `strategy.md`)
