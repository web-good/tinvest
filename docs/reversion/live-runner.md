...
3. Если заявки нет в state, а на бирже висит чужая/устаревшая заявка на этот инструмент
   (например, после `reconstruct`) — она отменяется (`Cancel`).
...
числится с непустым `stopOrderId` — runner считает, что сработала именно биржевая
стоп-заявка (а не ручная продажа), и шлёт `notifier.Exit` с **сохранёнными** `stopPrice` /
`stopReason` (а не пересчитанными заново), после чего запись удаляется из state. Явного
запроса к `GetStopOrders` по факту исполнения не требуется — достаточно того, что позиции
больше нет, а заявка на неё была.
...
- **STOP (`SL`/`ATRSL`/`TRAIL`):**
  - **Бэктест:** интрабарный триггер по `low` бара, гэп-исполнение как
    `min(stopLevel, nextOpen)` — знает open следующего бара и реалистично исполняет на гэпе
