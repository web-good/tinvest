...
2. **Подтверждения сигнала.** Покупочный tier (🟢 / 🟡) дополняется фильтром тренда (EMA200), бычьей дивергенцией RSI (фрактал Williams k=2) и подтверждением объёма (>1.5 × SMA20).
...
|---|---|---|
| 🥇 **Dividend (Gold)** — дивидендные «голубые фишки» | Полные тиры продажи (P80 🟠, P90 🔴, P95 🚨), частичные выходы по 1/3, ATR×2.0, EMA200-фильтр тренда ✅/🚫 | `shares.Dividend()` — 11 тикеров |
| 🥈 **Growth** — растущие акции | Единственный выход на P90 🔴 (полный), ATR×1.5, EMA200-фильтр тренда ✅/🚫 | `shares.Growth()` — 6 тикеров |

## Точки входа

- Чистая функция расчёта сигнала: `Detect()` — `internal/service/trading_strategy/golden_x/detector.go:19`.
...
| [alerts.md](alerts.md) | Расшифровка 12 emoji; шаблон сообщения; формат полей; блок легенды |
