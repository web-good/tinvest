...
Ядро стратегии — чистая функция `Detect()` в `internal/service/trading_strategy/golden_x/detector.go:18`. Никакого I/O, никакого времени, никакой телеметрии: на вход — массив закрытых недельных свечей, на выходе — `dto.Signal`. Всю обвязку (загрузку данных, дедупликацию, отправку в Telegram) делает обёртка `service.Trade`.
...
[свечи из Tinkoff]
       ↓
[фильтр незакрытой недели]   ← closedWeeklyCandles()
       ↓
...
### Шаг 1. Загрузка данных и фильтр незакрытой недели

`fetchWeeklyCandles()` (`detector.go:148`) тянет ~260 недельных свечей через gRPC Tinkoff Invest. Затем `closedWeeklyCandles()` (`trend_filter.go:70`) отбрасывает текущую неполную неделю — стратегия принципиально работает только на закрытых свечах. Граница недели — понедельник 00:00 в часовом поясе Москвы.
