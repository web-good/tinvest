...
3. **Фильтрация закрытых недель** — Эта фильтрация специфична для backtest — в live-пути аналогичный фильтр был удалён, и `Detect` теперь видит текущую формирующуюся неделю. В backtest по дизайну все свечи — закрытые исторические бары. `filterClosed()` (`cmd/backtest/main.go:197`). Понедельник 00:00 MSK как cutoff.
