...
//
// СХЕМА ПРОГОНОВ ШТАТНАЯ — та же, что у SVAV, SIBN и ELFV; адаптированную получали только IVAT
// (25/9/4) и DIAS (30/10/5), оба из-за короткой истории. История ровно 36 месяцев (получасовая серия
...
на английском, в стиле соседних: штатная схема 36/12/6 (как у SVAV, SIBN и ELFV; адаптированные
были только у IVAT и DIAS — из-за короткой истории), вердикт по планке, гибридная процедура тем и
её цена, дивидендные гэпы как главный эксплуатационный риск, нулевой запас истории (как у SVAV).
